// WARNING: DO NOT EDIT THIS FILE. THIS FILE IS MANAGED BY SPRING ROO.
// You may push code into the target .java compilation unit if you wish to edit any member(s).

package com.mcel2.domain;

import com.mcel2.domain.UserType;
import com.mcel2.domain.UserTypeDataOnDemand;
import com.mcel2.domain.UserTypeIntegrationTest;
import java.util.Iterator;
import java.util.List;
import javax.validation.ConstraintViolation;
import javax.validation.ConstraintViolationException;
import org.junit.Assert;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;
import org.springframework.transaction.annotation.Transactional;

privileged aspect UserTypeIntegrationTest_Roo_IntegrationTest {
    
    declare @type: UserTypeIntegrationTest: @RunWith(SpringJUnit4ClassRunner.class);
    
    declare @type: UserTypeIntegrationTest: @ContextConfiguration(locations = "classpath*:/META-INF/spring/applicationContext*.xml");
    
    declare @type: UserTypeIntegrationTest: @Transactional;
    
    @Autowired
    UserTypeDataOnDemand UserTypeIntegrationTest.dod;
    
    @Test
    public void UserTypeIntegrationTest.testCountUserTypes() {
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", dod.getRandomUserType());
        long count = UserType.countUserTypes();
        Assert.assertTrue("Counter for 'UserType' incorrectly reported there were no entries", count > 0);
    }
    
    @Test
    public void UserTypeIntegrationTest.testFindUserType() {
        UserType obj = dod.getRandomUserType();
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", obj);
        Long id = obj.getId();
        Assert.assertNotNull("Data on demand for 'UserType' failed to provide an identifier", id);
        obj = UserType.findUserType(id);
        Assert.assertNotNull("Find method for 'UserType' illegally returned null for id '" + id + "'", obj);
        Assert.assertEquals("Find method for 'UserType' returned the incorrect identifier", id, obj.getId());
    }
    
    @Test
    public void UserTypeIntegrationTest.testFindAllUserTypes() {
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", dod.getRandomUserType());
        long count = UserType.countUserTypes();
        Assert.assertTrue("Too expensive to perform a find all test for 'UserType', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 250);
        List<UserType> result = UserType.findAllUserTypes();
        Assert.assertNotNull("Find all method for 'UserType' illegally returned null", result);
        Assert.assertTrue("Find all method for 'UserType' failed to return any data", result.size() > 0);
    }
    
    @Test
    public void UserTypeIntegrationTest.testFindUserTypeEntries() {
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", dod.getRandomUserType());
        long count = UserType.countUserTypes();
        if (count > 20) count = 20;
        int firstResult = 0;
        int maxResults = (int) count;
        List<UserType> result = UserType.findUserTypeEntries(firstResult, maxResults);
        Assert.assertNotNull("Find entries method for 'UserType' illegally returned null", result);
        Assert.assertEquals("Find entries method for 'UserType' returned an incorrect number of entries", count, result.size());
    }
    
    @Test
    public void UserTypeIntegrationTest.testFlush() {
        UserType obj = dod.getRandomUserType();
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", obj);
        Long id = obj.getId();
        Assert.assertNotNull("Data on demand for 'UserType' failed to provide an identifier", id);
        obj = UserType.findUserType(id);
        Assert.assertNotNull("Find method for 'UserType' illegally returned null for id '" + id + "'", obj);
        boolean modified =  dod.modifyUserType(obj);
        Integer currentVersion = obj.getVersion();
        obj.flush();
        Assert.assertTrue("Version for 'UserType' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    
    @Test
    public void UserTypeIntegrationTest.testMergeUpdate() {
        UserType obj = dod.getRandomUserType();
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", obj);
        Long id = obj.getId();
        Assert.assertNotNull("Data on demand for 'UserType' failed to provide an identifier", id);
        obj = UserType.findUserType(id);
        boolean modified =  dod.modifyUserType(obj);
        Integer currentVersion = obj.getVersion();
        UserType merged = obj.merge();
        obj.flush();
        Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(), id);
        Assert.assertTrue("Version for 'UserType' failed to increment on merge and flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    
    @Test
    public void UserTypeIntegrationTest.testPersist() {
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", dod.getRandomUserType());
        UserType obj = dod.getNewTransientUserType(Integer.MAX_VALUE);
        Assert.assertNotNull("Data on demand for 'UserType' failed to provide a new transient entity", obj);
        Assert.assertNull("Expected 'UserType' identifier to be null", obj.getId());
        try {
            obj.persist();
        } catch (final ConstraintViolationException e) {
            final StringBuilder msg = new StringBuilder();
            for (Iterator<ConstraintViolation<?>> iter = e.getConstraintViolations().iterator(); iter.hasNext();) {
                final ConstraintViolation<?> cv = iter.next();
                msg.append("[").append(cv.getRootBean().getClass().getName()).append(".").append(cv.getPropertyPath()).append(": ").append(cv.getMessage()).append(" (invalid value = ").append(cv.getInvalidValue()).append(")").append("]");
            }
            throw new IllegalStateException(msg.toString(), e);
        }
        obj.flush();
        Assert.assertNotNull("Expected 'UserType' identifier to no longer be null", obj.getId());
    }
    
    @Test
    public void UserTypeIntegrationTest.testRemove() {
        UserType obj = dod.getRandomUserType();
        Assert.assertNotNull("Data on demand for 'UserType' failed to initialize correctly", obj);
        Long id = obj.getId();
        Assert.assertNotNull("Data on demand for 'UserType' failed to provide an identifier", id);
        obj = UserType.findUserType(id);
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'UserType' with identifier '" + id + "'", UserType.findUserType(id));
    }
    
}
